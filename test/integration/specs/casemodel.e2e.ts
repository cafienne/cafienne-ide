...
describe('Case modeler', async function () {
    ...
    it('Open case model - validate modeler elements', async function () {
        ...
        const humantask = await CaseModelerPage.shape('Human Task').dropInCanvas$({ x: 50, y: 40 });
        await CaseModelerPage.dropShape('Case Task', { x: 250, y: 40 });
        await CaseModelerPage.dropShape('Process Task', { x: 450, y: 40 });
        ...
        await stage.haloItem$('Entry Criterion').dropInCanvas$(milestone);
