...
export default class TaskDefinition extends TaskStageDefinition {
    ...
    get mappings() {
        if (!this._mappings) {
            this._mappings = /** @type {Array<ParameterMappingDefinition>} */ ([]);
        }
    ...
    createOutputParameterWithName(name) {
        const newParameter = /** @type {ParameterDefinition} */ (this.createDefinition(ParameterDefinition, undefined, this.generateUniqueOutputParameterName(name)));
    ...
    getParameterWithName(name, collection) {
        ...
        if (!existingParameter) {
            const newParameter = /** @type {ParameterDefinition} */ (this.createDefinition(ParameterDefinition, undefined, name));
     ...
     * @param {ParameterDefinition|undefined} taskParameter 
     * @param {ParameterDefinition|undefined} implementationParameter 
     * @returns {ParameterMappingDefinition}
     */
    createMapping(creator, sourceRef = '', targetRef = '', taskParameter = undefined, implementationParameter = undefined) {
        const newMapping = /** @type {*} */ (this.createDefinition(creator));
        ...
        if (this.implementationRef !== implementationRef) {
            ...
            this.inputMappings.forEach(mapping => {
                if (mapping.targetRef) {
                    ...
                    mapping.implementationParameter = this.implementationModel && this.implementationModel.findInputParameter(mapping.targetRef);
                }
            });
            ...
            this.outputMappings.forEach(mapping => {
                if (mapping.sourceRef) {
                    mapping.implementationParameter = this.implementationModel && this.implementationModel.findOutputParameter(mapping.sourceRef);
                }
            });
