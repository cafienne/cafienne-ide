...
export default class Migrator {
    ...
    constructor(definition) {
        ...
        const importNode = definition.importNode;
        ...
        if (XML.allElements(casePlan).filter(element => element.tagName === 'planItem' || element.tagName === 'discretionaryItem').length > 0) {
            console.groupCollapsed(`Parsing case plan of '${this.definition.file.fileName}' with ${XML.getElementsByTagName(importNode, 'planItem').length} plan items and ${XML.getElementsByTagName(importNode, 'discretionaryItem').length} discretionary items`);
