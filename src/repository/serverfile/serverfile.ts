...
export default class ServerFile<M extends ModelDefinition> {
    ...
    async rename(newFileName: string) {
        ...
        const filesToBeSaved = this.usage.map(file => {
            let changed = false;
            if (file.definition) {
        ...
        console.log(`Saving ${filesToBeSaved.length} files that had references to ${oldFileName}`)
        ...
        console.log(`Renamed ${oldFileName} to ${newFileName} at ${HHmmss}:${millis}`);
