...
export default class ShapeBox<V extends ModelCanvas = ModelCanvas> {
    modelCanvas: V;
    ...
    setDropHandler(dropHandler: (dragData: ShapeBoxDragData) => void, filter?: (dragData: ShapeBoxDragData) => boolean) {
        if (this.dragData) this.dragData.setDropHandler(<(dragData: ShapeBoxDragData) => void>dropHandler, <(dragData: ShapeBoxDragData) => boolean>filter);
    }
