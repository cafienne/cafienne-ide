...
export default class ValidateForm extends StandardForm {
    ...
    positionEditor() {
        ...
        const wBody = this.modelCanvas.editor.html.width();
        const hBody = this.modelCanvas.editor.html.height();
    ...
    showProblemsInForm(validator: Validator) {
        ...
        validator.errors.filter(e => e.modelDefinition === this.modelCanvas.caseDefinition).forEach(p => this.addProblemRow(p));
    ...
    addProblemRow(remark: Remark) {
        const link = remark.modelDefinition === this.modelCanvas.caseDefinition ? '' : '#' + remark.modelDefinition.file.fileName;
        ...
        html.on('click', e => this.modelCanvas.highlight(remark));
