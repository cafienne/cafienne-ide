...
export type ColumnConstructor<E extends ElementDefinition, R extends RowRenderer<E>> = {
...
export default class ColumnRenderer<E extends ElementDefinition, R extends RowRenderer<E>> {
    label: string;
    width: string;
    tooltip: string;
     ...
     */
    constructor(renderer: any, tooltip: string = '', label: string = '', width: string = '') {
    ...
    render(column: JQuery<HTMLTableCellElement>, row: R) {
        ...
        new (this.renderer as ColumnConstructor<E, R>)(row, column);
    }
}
