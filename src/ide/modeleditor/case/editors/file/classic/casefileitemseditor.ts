...
export default class CaseFileItemsEditor {
    ...
    constructor(public caseFileEditor: CaseFileEditor, public htmlParent: JQuery<HTMLElement>) {
        this.modelCanvas = caseFileEditor.modelCanvas;
        this.ide = this.modelCanvas.editor.ide;
    ...
    renderCaseFileModel() {
        ...
        this.modelCanvas.caseDefinition.caseFile.children.forEach(cfi => this.createNode(cfi));
    }
        ...
        if (from) {
            if (insert) {
                if (from.parentNode) {
                    ...
                    const parentDefinition = this.modelCanvas.caseDefinition.caseFile;
            ...
            newNode = this.createNode(this.modelCanvas.caseDefinition.caseFile.createChildDefinition());
        }
    ...
    async convertToType(e: JQuery.Event) {
        try {
            console.groupCollapsed(`Converting CaseFile structure of '${this.modelCanvas.editor.fileName}'`)
            await new CFIDConverter(this.modelCanvas).convert();
            console.groupEnd();
            console.log("Completed conversion, refreshing editor");
            this.modelCanvas.editor.refresh();
        } catch (error: any) {
            console.error(error);
            console.groupEnd();
    ...
    private getReferences(definitionElement: CaseFileItemDef) {
        const references: any[] = this.modelCanvas.items.filter(item => item.referencesDefinitionElement(definitionElement.id));
        // Also check whether the case parameters may be using the case file item
        if (this.modelCanvas.caseDefinition.input.find(p => p.bindingRef.references(definitionElement))) {
            references.push(this.modelCanvas.caseParametersEditor);
        } else if (this.modelCanvas.caseDefinition.output.find(p => p.bindingRef.references(definitionElement))) {
