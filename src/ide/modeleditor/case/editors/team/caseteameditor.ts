...
export default class CaseTeamEditor extends MovableEditor {
    ...
    constructor(cs: CaseView) {
        super(cs);
        this.caseTeam = this.modelView.caseDefinition.caseTeam;
    }
    ...
    private get removeTooltip(): string {
        ...
        if (teamFile) {
            const usage = teamFile.usage.filter((file: ServerFile) => file.definition?.id !== this.modelView.caseDefinition.id);
    ...
    renderHead() {
        ...
        new CaseTeamSelector(this.modelView.editor.ide.repository, teamSelector, this.caseTeam.caseTeamRef.toString(), (newTeamRef: string) => this.updateCaseTeamReference(newTeamRef), [{ option: '&lt;new&gt;', value: '<new>' }]);
    ...
    async convertTeam(): Promise<void> {
        ...
        const path = this.modelView.name.split('/');
    ...
    async removeTeam(): Promise<void> {
        if (!this.caseTeam.caseTeamRef.file) return;
        await this.modelEditor.ide.repositoryBrowser.delete(this.caseTeam.caseTeamRef.file, this.modelView.caseDefinition);
    }
        ...
        if (team) {
            ...
            if (oldSource !== newSource) {
                ...
                file.usage
                    ...
                    .map(file => this.modelView.editor.ide.editorRegistry.get(file))
                    .filter(caseEditor => caseEditor !== undefined).map(caseEditor => <CaseModelEditor>caseEditor)
                    .filter(caseEditor => caseEditor.modelView?.teamEditor.visible && caseEditor.modelView.teamEditor !== this)
                    .forEach(caseEditor => {
                        // console.log("Refreshing team in case " + caseEditor.file.fileName);
                        caseEditor.modelView?.teamEditor.renderData();
                    });
            }
        }
    }
    ...
    async openCreateCaseTeamModelDialog(): Promise<void> {
        ...
        const ide = this.modelView.editor.ide;
        if (!ide) return;
        ...
        if (file) {
            ...
            window.setTimeout(() => {
                if (file.usage.length) {
                    this.modelView.editor.ide.warning(
                    ...
                    if (confirm(text) === true) {
                        this.modelView.editor.ide.repository.delete(file.fileName);
                    }
                }
            }, 0);
        }
