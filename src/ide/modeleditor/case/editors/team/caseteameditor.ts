...
export default class CaseTeamEditor extends MovableEditor {
    ...
    constructor(cs: CaseView) {
        super(cs);
        this.caseTeam = this.modelCanvas.caseDefinition.caseTeam;
    }
    ...
    private get removeTooltip(): string {
        ...
        if (teamFile) {
            const usage = teamFile.usage.filter((file: ServerFile) => file.definition?.id !== this.modelCanvas.caseDefinition.id);
    ...
    renderHead() {
        ...
        new CaseTeamSelector(this.modelCanvas.editor.ide.repository, teamSelector, this.caseTeam.caseTeamRef.toString(), (newTeamRef: string) => this.updateCaseTeamReference(newTeamRef), [{ option: '&lt;new&gt;', value: '<new>' }]);
    ...
    async convertTeam(): Promise<void> {
        ...
        const path = this.modelCanvas.name.split('/');
    ...
    async removeTeam(): Promise<void> {
        if (!this.caseTeam.caseTeamRef.file) return;
        await this.modelEditor.ide.repositoryBrowser.delete(this.caseTeam.caseTeamRef.file, this.modelCanvas.caseDefinition);
    }
        ...
        if (team) {
            ...
            if (oldSource !== newSource) {
                ...
                file.usage
                    ...
                    .map(file => this.modelCanvas.editor.ide.editorRegistry.get(file))
                    .filter(caseEditor => caseEditor !== undefined).map(caseEditor => <CaseModelEditor>caseEditor)
                    .filter(caseEditor => caseEditor.modelCanvas?.teamEditor.visible && caseEditor.modelCanvas.teamEditor !== this)
                    .forEach(caseEditor => {
                        // console.log("Refreshing team in case " + caseEditor.file.fileName);
                        caseEditor.modelCanvas?.teamEditor.renderData();
                    });
            }
        }
    }
    ...
    async openCreateCaseTeamModelDialog(): Promise<void> {
        ...
        const ide = this.modelCanvas.editor.ide;
        if (!ide) return;
        ...
        if (file) {
            ...
            window.setTimeout(() => {
                if (file.usage.length) {
                    this.modelCanvas.editor.ide.warning(
                    ...
                    if (confirm(text) === true) {
                        this.modelCanvas.editor.ide.repository.delete(file.fileName);
                    }
                }
            }, 0);
        }
