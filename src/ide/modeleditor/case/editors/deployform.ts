...
export default class DeployForm extends StandardForm {
    ...
    <div class="deploy_content">
        ...
        const model = this.modelEditor.ide.repository.get(this.modelCanvas.editor.fileName);
        if (model && model.usage.length > 0) {
            ...
            const whereUsedCounter = `Case '${this.modelCanvas.caseDefinition.file.name}' is used in ${model.usage.length} other model${model.usage.length == 1 ? '' : 's'}`;
    ...
    async deploy() {
        const deployment = new Definitions(this.modelCanvas.caseDefinition);
        await this.modelEditor.ide.repository.deploy(deployment).catch(error => {
            ...
            this.modelCanvas.editor.ide.danger('Deploy of CMMN model ' + this.modelCanvas.name + ' failed');
        });
    ...
    viewCMMN() {
        ...
        const deploy = new Definitions(this.modelCanvas.caseDefinition);
