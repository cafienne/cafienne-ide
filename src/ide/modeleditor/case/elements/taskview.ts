...
export default abstract class TaskView<TD extends TaskDefinition = TaskDefinition> extends TaskStageView<TD> {
    ...
    setDropHandlers() {
        ...
        this.modelView.editor.ide.repositoryBrowser.setDropHandler(dragData => this.changeTaskImplementation(dragData.file), dragData => this.supportsFileTypeAsImplementation(dragData));
    }
    ...
    async generateNewTaskImplementation() {
        ...
        if (existingModel) {
            ...
            const fileName = await this.modelView.editor.ide.createNewModel(this.fileType, potentialImplementationName, this.definition.documentation.text);
    ...
    async changeTaskImplementation(file: ServerFile, updateTaskName: boolean = false) {
        ...
        await file.load();
        if (!file.definition) {
            this.modelView.editor.ide.warning('Could not read the definition ' + file.fileName + ' which is referenced from the task ' + this.name
            );
            return;
        }
