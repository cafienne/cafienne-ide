...
export default abstract class SentryView<CD extends CriterionDefinition = CriterionDefinition> extends CMMNElementView<CD> {
    ...
    updateConnectorLabels() {
        const style = this.modelView.diagram.connectorStyle;
    ...
    moving(x: number, y: number) {
        const parentElement = this.modelView.graph?.getCell(this.parent.xyz_joint.id);
    ...
    __getConnectableElements(): CMMNElementView[] {
        const connectableElements = this.modelView.items.filter(cmmnElementView => {
    ...
    __getOnPart(connector: Connector<CMMNElementView>): OnPartDefinition<any> | undefined {
        ...
        return this.definition.caseFileItemOnParts.find(onPart => {
            const casefileElement = this.modelView.getCaseFileItemElement(onPart.sourceRef.value);
