...
export default class StageView<SD extends StageDefinition = StageDefinition> extends TaskStageView<SD> {
    static create(stage: StageView, x: number, y: number): StageView {
        ...
        const shape = stage.modelCanvas.diagram.createShape(x, y, 420, 140, definition.id);
        return new StageView(stage.modelCanvas, stage, definition, shape);
    }
    ...
    setDropHandlers() {
        ...
        this.modelCanvas.editor.ide.repositoryBrowser.setDropHandler(dragData => this.addTaskModel(dragData), dragData => dragData.file instanceof CaseFile || dragData.file instanceof HumanTaskFile || dragData.file instanceof ProcessFile);
        // ... and case file items to be dropped from the cfiEditor
        this.modelCanvas.cfiEditor.setDropHandler(dragData => this.addCaseFileItem(dragData));
    }

    removeDropHandlers() {
        super.removeDropHandlers();
        this.modelCanvas.editor.ide.repositoryBrowser.removeDropHandler();
        this.modelCanvas.cfiEditor.removeDropHandler();
    }
    ...
    addCaseFileItem(dragData: CaseFileItemDragData) {
        ...
        const coor = this.modelCanvas.getCursorCoordinates(evt);
    ...
    resetChildren() {
        ...
        const allCaseItems = this.modelCanvas.items.filter(item => !item.isPlanningTable && !item.isCriterion);
    ...
    addPlanItem(definition: PlanItem) {
        ...
        if (!this.__childElements.find(planItemView => planItemView.definition.id == definition.id)) {
            ...
            const shape = this.modelCanvas.diagram.getShape(definition);
            if (!shape) {
    ...
    createPlanItemView(definition: PlanItem, shape: ShapeDefinition) {
        if (definition instanceof HumanTaskDefinition) {
            ...
            return new StageView(this.modelCanvas, this, definition, shape);
