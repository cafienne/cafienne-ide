...
export default class Properties<V extends CMMNElementView = CMMNElementView> extends MovableEditor {
    ...
    show(focusNameField = false) {
        ...
        this.modelCanvas.items.filter(item => item != this.view).forEach(item => item.propertiesView.hide());
    ...
    positionEditor() {
        ...
        if (!this.pinned) {
            ...
            const canvasOffset = (this.view.modelCanvas as any).svg.offset() || { left: 0, top: 0 };
