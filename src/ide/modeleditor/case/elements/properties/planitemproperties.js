...
export default class PlanItemProperties extends Properties {
    ...
    getRolesAsHTMLSelect(currentRoleId, buttonClass) {
        let existingRolesAsOptions = '';
        if (this.case.caseDefinition.caseTeam.isOldStyle) {
            existingRolesAsOptions = this.case.caseDefinition.caseTeam.roles.map(role => `<option value="${role.id}" ${role.id == currentRoleId ? ' selected' : ''}>${role.name}</option>`).join('');
        } else {
            /** @type CaseTeamModelDefinition */
            const definition = this.case.editor.ide.repository.get(this.case.caseDefinition.caseTeam.caseTeamRef)?.definition;
            /** @type Array<CaseRoleDefinition> */
            const roles = definition?.roles;
            if (roles) {
                existingRolesAsOptions = roles.map(caseRole => `<option value="${caseRole.name}" ${caseRole.name == currentRoleId ? ' selected' : ''}>${caseRole.name}</option>`).join('');
            }
        }
