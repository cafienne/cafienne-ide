...
export default class CaseFileItemProperties extends Properties<CaseFileItemView> {
    renderData() {
        ...
        const cfi = this.view.modelView.caseDefinition.getElement(caseFileItemId) as CaseFileItemDef | undefined;
        ...
        html.find('.zoombt').on('click', e => this.view.modelView.cfiEditor.open((cfi: CaseFileItemDef) => this.changeContextRef(html, cfi)));
        html.find('.removeReferenceButton').on('click', e => this.changeContextRef(html));
        html.on('pointerover', e => {
            e.stopPropagation();
            this.view.modelView.cfiEditor.setDropHandler((dragData: { item: CaseFileItemDef }) => this.changeContextRef(html, dragData.item));
        });
        html.find('.zoomRow').on('pointerout', e => this.view.modelView.cfiEditor.removeDropHandler());
