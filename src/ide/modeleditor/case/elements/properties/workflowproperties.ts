...
export default class WorkflowProperties extends TaskProperties<HumanTaskView> {
    ...
    addAssignmentField() {
        ...
        html.find('.zoombt').on('click', () => this.view.modelCanvas.cfiEditor.open(cfi => this.change(this.humanTaskDefinition.workflow.assignment!, 'contextRef', cfi.id)));
        ...
        html.find('.zoomRow').on('pointerover', (e: JQuery.Event) => {
            e.stopPropagation();
            this.view.modelCanvas.cfiEditor.setDropHandler((dragData: any) => {
                const newContextRef = dragData.item.id;
                this.change(this.humanTaskDefinition.workflow.assignment!, 'contextRef', newContextRef);
            });
        });
        html.find('.zoomRow').on('pointerout', () => this.view.modelCanvas.cfiEditor.removeDropHandler());
        this.htmlContainer.append(html);
        return html;
    }
    ...
    addDueDateField() {
        ...
        html.find('.zoombt').on('click', () => this.view.modelCanvas.cfiEditor.open(cfi => this.change(this.humanTaskDefinition.workflow.dueDate!, 'contextRef', cfi.id)));
    ...
    addFourEyesField() {
        ...
        const tasks = this.modelCanvas.caseDefinition.getAllPlanItems().filter((item: any) => item instanceof HumanTaskDefinition);
