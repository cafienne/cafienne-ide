...
export default class WorkflowProperties extends TaskProperties<HumanTaskView> {
    ...
    addAssignmentField() {
        ...
        html.find('.zoombt').on('click', () => this.view.modelView.cfiEditor.open(cfi => this.change(this.humanTaskDefinition.workflow.assignment!, 'contextRef', cfi.id)));
        ...
        html.find('.zoomRow').on('pointerover', (e: JQuery.Event) => {
            e.stopPropagation();
            this.view.modelView.cfiEditor.setDropHandler((dragData: any) => {
                const newContextRef = dragData.item.id;
                this.change(this.humanTaskDefinition.workflow.assignment!, 'contextRef', newContextRef);
            });
        });
        html.find('.zoomRow').on('pointerout', () => this.view.modelView.cfiEditor.removeDropHandler());
        this.htmlContainer.append(html);
        return html;
    }
    ...
    addDueDateField() {
        ...
        html.find('.zoombt').on('click', () => this.view.modelView.cfiEditor.open(cfi => this.change(this.humanTaskDefinition.workflow.dueDate!, 'contextRef', cfi.id)));
    ...
    addFourEyesField() {
        ...
        const tasks = this.modelView.caseDefinition.getAllPlanItems().filter((item: any) => item instanceof HumanTaskDefinition);
