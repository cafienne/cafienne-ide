...
export default class CasePlanView extends StageView<CasePlanDefinition> {
    ...
    referencesDefinitionElement(definitionId: string) {
        ...
        if (this.modelCanvas.caseDefinition.input.find(p => p.bindingRef.references(definitionId))) {
            return true;
        }
        if (this.modelCanvas.caseDefinition.output.find(p => p.bindingRef.references(definitionId))) {
            return true;
        }
    ...
    __delete() {
        super.__delete();
        delete this.modelCanvas.casePlanModel;
    }
