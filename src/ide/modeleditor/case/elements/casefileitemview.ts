...
export default class CaseFileItemView extends CMMNElementView<CaseFileItemDef> {
    ...
    static create(stage: StageView, x: number, y: number, definition?: CaseFileItemDef) {
        definition = definition || CaseFileItemDef.createEmptyDefinition(stage.modelCanvas.caseDefinition);
        const shape = stage.modelCanvas.diagram.createShape(x, y, 25, 40, definition.id);
    ...
    setDefinition(definition?: CaseFileItemDef) {
        this.definition = definition ? definition : CaseFileItemDef.createEmptyDefinition(this.modelCanvas.caseDefinition);
