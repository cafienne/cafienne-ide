    ...
    showPlanningTable() {
        if (this.definition.planningTable) {
            if (!this.planningTableView) {
                ...
                const shape = this.modelView.diagram.getShape(this.definition.planningTable) || this.modelView.diagram.createShape(position.x, position.y, 24, 16, this.definition.planningTable.id);
