...
export default class CaseModelEditor extends ModelEditor {
    ...
    constructor(public ide: IDE, public file: CaseFile) {
        ...
        this.ideCaseFooter.find('.validateLabel').on('click', () => this.modelView && this.modelView.validateForm.show());
    }
    ...
    loadDefinition(caseDefinition: CaseDefinition | undefined = this.caseFile.definition) {
        ...
        if (this.modelView) {
            this.modelView.delete();
        }

        // Create a new case renderer on the definition and dimensions
        this.modelView = new CaseView(this, this.htmlContainer, caseDefinition);
        ...
        window.setTimeout(() => this.modelView?.runValidation(), 100);
    }
    ...
    keyStrokeHandler(e: JQuery.KeyDownEvent) {
        ...
        const selectedElement = this.modelView.selectedElement;
        switch (e.keyCode) {
            case 27: // esc
                ...
                if (this.modelView.sourceEditor.visible) {
                    this.modelView.sourceEditor.close();
            ...
            case 46: //del
                if (!visibleMovableEditor && selectedElement) {
                    this.modelView.__removeElement(selectedElement);
                    this.modelView.clearSelection();
                }
                break;
            case 37: //arrow left;
    ...
    saveModel() {
        ...
        if (this.modelView) {
            this.modelView.runValidation();
            this.undoManager.saveCaseModel(this.modelView.caseDefinition, this.modelView.dimensions!);
        }
    }

    onShow() {
        this.ideCaseFooter.css('display', 'block');
        this.modelView && this.modelView.onShow();
    }
