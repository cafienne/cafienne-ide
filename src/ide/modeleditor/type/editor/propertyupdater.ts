...
export default class PropertyUpdater {
    ...
    static async updateNameChangeInOtherModels(renderer: PropertyRenderer, newName: string) {
        ...
        console.group(`Repository: updating other models after name change in ${renderer.property.modelDefinition.file.fileName}: '${oldPath}' ==> '${newPath}'`);
    ...
    static async updateCaseDefinition(caseDefinition: CaseDefinition, refs: CaseFileItemTypeDefinition[], renderer: PropertyRenderer): Promise<any> {
        const caseFileItemReferences = refs.map((cfi: CaseFileItemTypeDefinition) => cfi.searchInboundReferences().filter(ref => ref.modelDefinition.file.name === cfi.modelDefinition.file.name)).flat();
