...
export default class PropertyUsage {
    ...
    static checkPropertyDeletionAllowed(renderer: PropertyRenderer) {
        const getCaseReferences = (property: SchemaPropertyDefinition) => Util.removeDuplicates(property.searchInboundReferences().filter(element => element instanceof CaseFileItemTypeDefinition).map(cftd => cftd.searchInboundReferences()).flat());
