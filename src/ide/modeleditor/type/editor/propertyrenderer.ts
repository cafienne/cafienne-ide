...
export default class PropertyRenderer extends TypeRenderer<SchemaPropertyDefinition> {
    ...
    attachEventHandlers() {
        ...
        this.htmlContainer.find('.cfi-icon').on('pointerdown', e => {
            if (this.property.isComplexType && this.editor.modelCanvas) {
                ...
                const cfi: CaseFileItemDef = this.editor.modelCanvas.caseDefinition.createDefinition(CaseFileItemDef, undefined, this.path, this.name);
                this.editor.modelCanvas.cfiEditor.startDragging(cfi);
            }
        });
