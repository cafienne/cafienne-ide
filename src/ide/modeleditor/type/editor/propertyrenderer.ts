...
export default class PropertyRenderer extends TypeRenderer<SchemaPropertyDefinition> {
    ...
    attachEventHandlers() {
        ...
        this.htmlContainer.find('.cfi-icon').on('pointerdown', e => {
            if (this.property.isComplexType && this.editor.modelView) {
                ...
                const cfi: CaseFileItemDef = this.editor.modelView.caseDefinition.createDefinition(CaseFileItemDef, undefined, this.path, this.name);
                this.editor.modelView.cfiEditor.startDragging(cfi);
            }
        });
